#!/usr/bin/env python3
"""
Direct patch: search for specific decrypt call patterns and replace with const-string.
"""
...
def patch_smali_files(smali_dir, output_dir, strings):
    ...
    for smali_file in Path(smali_dir).rglob('*.smali'):
        ...
        # Pattern 1: Direct const values before invoke-static
        # Look for sequences like:
        # const/16 vX, 0xSTART
        # const/4 vY, 0xXOR
        # const/16 vZ, 0xLENGTH
        # invoke-static {vA, vX, vY, vZ}, Class;->method([SIII)Ljava/lang/String;
        # move-result-object vR
        
        # Find all invoke-static with [SIII
        for match in re.finditer(r'invoke-static\s+\{([^}]+)\},\s+\S+;->\w+\(\[SIII\)Ljava/lang/String;', content):
            call_pos = match.start()
            regs = match.group(1)
            
            # Look backwards for const instructions
            before = content[max(0, call_pos-500):call_pos]
            
            # Extract const values
            consts = {}
            for cm in re.finditer(r'const/(?:4|16)\s+(v\d+|v\d+),\s+#?(0x[0-9a-fA-F]+|-?\d+)', before):
                reg = cm.group(1)
                val = int(cm.group(2), 16) if cm.group(2).startswith('0x') else int(cm.group(2))
                consts[reg] = val
            
            # Also check for move/from16
            ...
            if len(invoke_regs) == 4:
                # invoke_regs[0] = array, [1] = start, [2] = xor, [3] = length
                ...
                if start_val is not None and xor_val is not None and len_val is not None:
                    ...
                    if key in strings:
                        ...
                        # Find move-result-object after the call
                        ...
                        if result_match:
                            ...
                            # Find the start of the sequence (from the first const or sget-object)
                            ...
                            # Find end (after move-result-object)
